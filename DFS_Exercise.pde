 ...
 TODO: 
    1. Try to understand how this Breadth-first Search (BFS) implementation works.
       ...
       How do I represent nodes? How do I represent edges?
          Nodes are represented in an arrayList. The edges are stored as an arrayList called the fringe.
       What is the purpose of the visited list? What about the parent list?
          The visited list is beneficial because it tells us which nodes we have already visited, which is important for graphs because we don't want to get stuck in a loop.
          The parent list is there to tell us which node we visited before coming to the current node. It is the FIFO part of the algorithm.
       What is getting added to the fringe? In what order?
         The fringe stores nodes with unexplored actions, which are stored in a FIFO order. 
       How do I find the path once I've found the goal?
         The path is just the nodes in the queue. 
...
neighbors[3].add(4); neighbors[2].add(6); //3 -> 4 & 6
